...
# Hybrid Revolution and Coati Optimization Algorithm (HRCOA) Starting Line No. 60
def Proposed(X, objective, lb, ub, max_iter):
    ...
    for t in range(max_iter):
        ...
        currentfit = F[idx]
        meanfit = np.mean(F)
        if currentfit > meanfit:
            # ROA compute candidate positions
